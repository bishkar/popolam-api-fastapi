...
class OrderCRUD(BaseCRUD[Order, OrderCreate, any]):
    ...
    async def retrieve(self, unique_id: int, db_session: AsyncSession) -> List[Order]:
        statement = select(Order).where(Order.customer == unique_id)
        result = await db_session.execute(statement)
        orders = result.scalars().all()
        return orders
    
    async def create(self, order: OrderCreate, db_session: AsyncSession) -> Order: 
        try:
    ...
    async def delete(self, user: User, order_id: int, db_session: AsyncSession) -> HTTPException:
